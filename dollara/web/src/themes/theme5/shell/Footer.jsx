...
const QUICK_NAV = [
  ...
  { label: 'Direct Support', href: '/support/chat', arrow: true },
...
export function Theme5Footer() {
  ...
  return (
    <footer className="mt-4 overflow-hidden rounded-xl bg-white p-5 shadow-sm sm:p-7">
      <div className="grid gap-8 lg:grid-cols-[1.4fr_0.8fr_1.2fr]">
        {/* ── Brand blurb ── */}
        <div>
          ...
          <p className="mt-4 max-w-md text-sm leading-relaxed text-[#475569]">
            <span className="font-black text-[#0f1b33]">{name}</span> is the best platform for live and
            uninterrupted online betting for sports, Live 24hr betting with a wide spectrum of sports such
            as Cricket, Soccer, Horse Racing, Kabaddi,{' '}
          ...
          <h4 className="mt-5 font-display text-sm font-black uppercase tracking-wide text-[#0f1b33]">
            100% Safe &amp; Instant Payments
          </h4>
          <p className="mt-1.5 max-w-md text-sm text-[#475569]">
            You can make payments and receive earnings instantly via your UPI ID…
          </p>

          <div className="mt-5 flex flex-wrap items-center gap-2">
            <span className="rounded-lg border border-[#f4547a]/40 px-2.5 py-1.5 text-[0.6rem] font-black uppercase tracking-wide text-[#f4547a]">
              18+ Only
            </span>
            <span className="grid h-8 w-8 place-items-center rounded-lg border border-black/10 text-[0.65rem] font-black text-[#64748b]">
              G
            </span>
            <span className="grid h-8 w-8 place-items-center rounded-lg border border-black/10 text-[0.65rem] font-black text-[#64748b]">
              GT
            ...
            <Link
              href="/app"
              className="inline-flex items-center gap-2 rounded-lg bg-[#22a34a] px-5 py-2.5 text-xs font-black uppercase tracking-wide text-white shadow-sm transition hover:brightness-110"
            >
        ...
        <div>
          ...
          <div className="mt-4 rounded-xl border border-black/[0.07] bg-[#f8fafc] p-4 text-center">
            <p className="text-[0.6rem] font-black uppercase tracking-[0.18em] text-[#94a3b8]">
              Payment Options
            </p>
            <div className="mt-3 flex flex-wrap items-center justify-center gap-2">
              {PAYMENTS.map((p) => (
                <span
                  key={p}
                  className="rounded-lg border border-black/[0.07] bg-white px-3 py-1.5 text-[0.7rem] font-black text-[#0f1b33] shadow-sm"
                >
                  {p}
                </span>
              ))}
            </div>
            <p className="mt-3 text-[0.6rem] font-bold uppercase tracking-wide text-[#94a3b8]">
              {PAYMENTS.join(' / ')}
            </p>
          </div>
        </div>
      ...
      {/* ── Legal strip ── */}
      <div className="mt-7 border-t border-black/[0.06] pt-4 text-center">
        ...
        <p className="mt-2 text-[0.7rem] font-black uppercase tracking-wide text-[#1d4ed8]">

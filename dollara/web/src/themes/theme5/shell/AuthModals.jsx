...
//   - Register → phone → OTP → full name + password.
...
import { t5Input, t5Select, t5BtnPrimary } from '../components/ui';
  ...
  return (
    <AuthModal onClose={close} brandName={brandName} headline="Create Account">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <select value={channel} onChange={(e) => setChannel(e.target.value)} className={`${t5Select} pl-11`}>
      ...
      {step === 'otp' && (
        <div className="space-y-4">
          {devOtp && (
            <p className="rounded-lg border border-[#1d4ed8]/25 bg-[#eff4ff] px-3 py-2 text-center text-sm text-[#1d4ed8]">
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`${t5Input} pl-11 text-center text-2xl tracking-[0.5em]`}
            />
          </div>
          <button type="button" onClick={register} disabled={loading || otp.length !== 6} className={`${t5BtnPrimary} w-full`}>
            {loading ? 'Creating account…' : 'Verify & Register'}
          </button>
          <button type="button" onClick={() => setStep('phone')} className="w-full text-sm text-[#64748b] hover:text-[#0f1b33]">

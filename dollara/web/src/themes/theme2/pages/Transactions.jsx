...
// Theme2 Transactions — dark-navy / gold. The player's money ledger off the
// shared wallet API (/api/v1/wallet/transactions): deposits, withdrawals, bonus
// credits, bet settlements, refunds and adjustments, newest first.
//
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
          ...
          <button
            ...
            className={`rounded-full px-3.5 py-1.5 text-xs font-black uppercase tracking-wide transition ${
              filter === f.value
                ...
                : 'border border-white/10 bg-[#0d1420] text-slate-400 hover:border-amber-400/50 hover:text-amber-400'
            }`}
          >
            {f.label}
          </button>
      ...
      {loading ? (
        ...
        <T2Card className="px-4 py-12 text-center text-sm font-semibold text-rose-400">{error}</T2Card>
      ) : visible.length === 0 ? (
              ...
              <tbody>
                {visible.map((t) => (
                  <tr key={t.id} className="border-b border-white/5 transition last:border-0 hover:bg-white/[0.03]">
                    ...
                    <td className="px-5 py-3 font-mono text-[0.7rem] text-slate-500">{t.reference_number || '—'}</td>
...
function AmountCell({ tx }) {
  ...
  const tone = !isSettled(tx.status) ? 'text-slate-500' : credit ? 'text-emerald-400' : 'text-rose-400';
  return (
...
function StatusPill({ status }) {
  ...
  const cls =
    tone === 'good' ? 'bg-emerald-500/15 text-emerald-400' : tone === 'bad' ? 'bg-rose-500/15 text-rose-400' : 'bg-amber-500/15 text-amber-400';
  return <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-black capitalize ${cls}`}>{status}</span>;
}

...
// Theme3 Bet History — play/session records + aggregate P&L off the shared
// games API (/api/v1/games/history, /api/v1/games/pnl), rendered in VELPLAY's
// cream/gold style. Each row expands into its round-by-round detail
...
export default function Theme3BetHistory() {
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-3 py-4 sm:px-5 sm:py-8">
      <h1 className="font-display text-2xl font-black text-[#1b1726] sm:text-3xl">Bet History</h1>
      ...
      <div className="mb-3 mt-8 flex items-center justify-between">
        <h2 className="font-display text-lg font-black text-[#1b1726]">Sessions</h2>
                  ...
                  return (
                    <Fragment key={r.session_uid}>
                      <tr
                        ...
                        className="cursor-pointer border-b border-black/[0.05] transition last:border-0 hover:bg-[#faf6ec]/60"
                      >
                        <td className="px-5 py-3 font-black text-[#1b1726]">{r.game_name}</td>
                        <td className="px-5 py-3 capitalize text-[#6b6579]">
                          {(r.category || '—').replace(/_/g, ' ')}
                        </td>
                        <td className="px-5 py-3 text-[#6b6579]">
                          {formatDateTime(r.last_played_at || r.created_at)}
                        </td>
                        <td className="px-5 py-3 text-center text-[#6b6579]">
                          {r.rounds}
                          {r.pending_rounds > 0 && (
                            <span className="ml-1 text-[0.65rem] text-[#9a7a24]">
                              ({r.pending_rounds} open)
                            </span>
                          )}
                        </td>
                        <td className="px-5 py-3 text-right tabular-nums text-[#1b1726]">{inr(r.total_bet)}</td>
                        <td className="px-5 py-3 text-right tabular-nums text-[#1b1726]">{inr(r.total_win)}</td>
                        <td className="px-5 py-3 text-right">
                          <ResultCell record={r} />
                        </td>
                        <td className="px-2 py-3 text-[#9a94a8]">
                          <ChevronDown className={`h-4 w-4 transition-transform ${open ? 'rotate-180' : ''}`} />
                        </td>
                      </tr>
                      {open && (
                        <tr className="border-b border-black/[0.05]">
                          <td colSpan={8} className="bg-[#faf6ec] px-5 py-4">
...
function ResultCell({ record }) {
  if (record.result === 'pending') {
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-[#f3ead4] px-2.5 py-0.5 text-xs font-black text-[#9a7a24]">
  ...
  return (
    <span className={`font-black tabular-nums ${up ? 'text-[#1c8a52]' : 'text-[#c23a3e]'}`}>
      {up ? '+' : '−'}
...
function RoundDetails({ detail, onSelectRound }) {
  if (!detail || detail.loading) {
    return (
      <p className="flex items-center gap-2 text-sm text-[#9a94a8]">
        <Loader2 className="h-4 w-4 animate-spin" /> Loading rounds…
      </p>
    );
  }
  if (detail.error) {
    return <p className="text-sm font-semibold text-[#c23a3e]">{detail.error}</p>;
  }
  if (!detail.rounds.length) {
    return <p className="text-sm text-[#9a94a8]">No round details recorded yet.</p>;
  }
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[880px] text-xs">
        <thead>
          <tr className="text-left uppercase tracking-wide text-[#9a94a8]">
        ...
        <tbody className="text-[#1b1726]">
          {detail.rounds.map((rd) => (
            ...
            >
              <td className="py-2 pr-4 font-mono text-[0.7rem] text-[#6b6579]">
                {rd.game_round || rd.serial_number}
              </td>
              <td className="py-2 pr-4">{rd.game_name || '—'}</td>
              <td className="py-2 pr-4 capitalize text-[#6b6579]">{(rd.category || '—').replace(/_/g, ' ')}</td>
              <td className="py-2 pr-4 text-[#6b6579]">{formatDateTime(rd.created_at)}</td>
              <td className="py-2 pr-4 text-[#6b6579]">{rd.settled_at ? formatDateTime(rd.settled_at) : '—'}</td>
              ...
              <td className="py-2 pr-4 text-right tabular-nums text-[#6b6579]">
                {rd.balance_after == null ? '—' : inr(rd.balance_after)}
              </td>
              <td className="py-2 text-right">
                {rd.result === 'pending' ? (
                  <span className="font-bold text-[#9a7a24]">Pending</span>
                ) : (
                  <span className={`font-bold tabular-nums ${rd.profit_loss >= 0 ? 'text-[#1c8a52]' : 'text-[#c23a3e]'}`}>
                    {rd.profit_loss >= 0 ? '+' : '−'}
...
function RoundDetailModal({ round, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <button aria-label="Close" onClick={onClose} className="absolute inset-0 cursor-default bg-black/30 backdrop-blur-sm" />

      <div className="relative z-10 w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="relative bg-gradient-to-br from-[#2a2140] via-[#241b3a] to-[#170f28] px-6 py-5">
          <button
            onClick={onClose}
            aria-label="Close"
          ...
          <p className="font-display text-lg font-black uppercase leading-none tracking-tight text-[#e9c56b]">
            Round details
          </p>
          ...
          <dl className="space-y-3">
            {rows.map(([label, value]) => (
              <div key={label} className="flex items-center justify-between gap-4 text-sm">
                <dt className="text-[0.65rem] font-bold uppercase tracking-wide text-[#9a94a8]">{label}</dt>
                <dd className="text-right font-semibold tabular-nums text-[#1b1726]">{value}</dd>
              </div>
            ))}
            <div className="flex items-center justify-between gap-4 border-t border-black/[0.06] pt-3 text-sm">
              <dt className="text-[0.65rem] font-bold uppercase tracking-wide text-[#9a94a8]">Result</dt>
              <dd>
                {isPending ? (
                  <span className="inline-flex items-center gap-1 rounded-full bg-[#f3ead4] px-2.5 py-0.5 text-xs font-black text-[#9a7a24]">
                    <Clock className="h-3 w-3" /> Pending
